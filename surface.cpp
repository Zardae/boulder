...
// Basic Circle, without Antialiasing
void Surface::Circle(int x, int y, int radius, Pixel color)
{
	...
	for (int ix = 0; ix <= 2 * radius + 1; ix++)
	{
		...
		{
			if (pow((x + ix - centerX), 2) + pow((y + iy - centerY), 2) <= pow(radius, 2) + 2)	// Cheated a bit with the "+ 2" int the end. This was added to give a bit more area to the lines going directly from the center
			{
				Plot(x + ix, y + iy, color);
			}
		}
	}
}
